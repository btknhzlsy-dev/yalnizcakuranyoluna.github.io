...
// --- GÖRÜNÜM YÖNETİCİSİ ---
// --- GÖRÜNÜM YÖNETİCİSİ ---
function gorunumuGoster(gorunumId, addToHistory = true, scrollToTop = true) {
    // Şu anki aktif görünümü ve modalları bul ve geçmişe ekle
    if (addToHistory) {
        const activeView = document.querySelector('.view.active');
        if (activeView && activeView.id !== gorunumId) {
            // Aktif modalları bul
            const activeModals = Array.from(document.querySelectorAll('.fixed.inset-0.active')).map(m => m.id);
            viewHistory.push({
                viewId: activeView.id,
                modals: activeModals,
                scrollPosition: window.scrollY // YENİ: Kaydırma pozisyonunu kaydet
            });
        }
    }

    // Tüm modalları kapat (yeni görünüme geçerken temiz sayfa)
...
function goBack() {
    if (viewHistory.length > 0) {
        ...
        // Görünümü geri yükle (geçmişe eklemeden ve en üste kaydırmadan)
        gorunumuGoster(previousState.viewId, false, false);

        // Modalları geri yükle
        if (previousState.modals && previousState.modals.length > 0) {
            previousState.modals.forEach(modalId => {
            ...
            if (ayet) {
                ...
                ayetKarti.innerHTML = `
                    <div class="cursor-pointer" data-surah-id="${sureIdStr}" data-verse-id="${ayetIdStr}">
                        ...
                        <p class="font-bold text-amber-900 mt-3">${ayet.anaCeviri || 'Çeviri bulunamadı.'}</p>
...
function aramaSonuclariniGoster(sonuclar, sorgu, aramaTipi) {
    ...
    sonuclar.forEach(sonuc => {
        ...
        if (aramaTipi === 'translation') {
            ...
            ayetMetniHTML = `<p class="font-bold text-amber-900 mt-3">${sonuc.ayet.anaCeviri || 'Çeviri bulunamadı.'}</p>`;
            ...
            ayetMetniHTML = `
                ...
                <p class="font-bold text-amber-900 mt-3">${sonuc.ayet.anaCeviri || 'Çeviri bulunamadı.'}</p>
            `;
        }
...
function ayetDetayPenceresiniGoster(sureId, ayetId) {
    ...
    veri.kelimeler.forEach(kelime => {
        ...
        if (arapcaAciklamasi) {
            ...
            // GÜNCELLEME: Alt çizgi rengi sarı yapıldı ve genişlik metin kadar (w-max / inline-block)
            arapcaButonu.className = 'hover:text-emerald-600 border-b-2 border-yellow-400 border-dotted cursor-pointer text-right inline-block';
...
function kokPenceresiniGoster(kok, anlam) {
    ...
    if (kurandaAratButonu) {
        ...
        kurandaAratButonu.innerHTML = `
            <svg class="w-3 h-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" />
            </svg>
            Kelimeyi Kur'an'da Arat
        `;

        // Eski event listener'ları temizlemek için klonlama
        ...
        kurandaAratButonu.addEventListener('click', () => {
            // NOT: Modalları burada kapatmıyoruz. gorunumuGoster('search-view') çağrıldığında
            // mevcut durum (modallar dahil) geçmişe kaydedilecek ve sonra modallar kapatılacak.

            // Arama inputuna kökü yaz (# ile başlayarak)
            aramaInputu.value = '#' + kok;
...
function arapcaKelimePenceresiniGoster(arapcaKelime, aciklama) {
    ...
    if (kurandaAratButonu) {
        ...
        kurandaAratButonu.addEventListener('click', () => {
            ...
            // Arama inputuna kelimeyi yaz (Normalleştirilmiş haliyle)
            aramaInputu.value = normalizeArabic(arapcaKelime);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (aramaGeriButonu) {
        aramaGeriButonu.addEventListener('click', goBack);
    }
    if (aramaFormu) {
    ...
    // YENİ: ESC tuşu ile geri gitme ve modalları kapatma
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            // 1. Modallar (Varsa kapat butonlarını tetikle)
            if (kokPenceresi && kokPenceresi.classList.contains('active')) {
                kokPenceresiKapatButonu.click();
                return;
            }
            if (ayetDetayPenceresi && ayetDetayPenceresi.classList.contains('active')) {
                detayPenceresiKapatButonu.click();
                return;
            }

            // 2. Görünümler (Aktif görünüme göre geri butonunu tetikle)
            if (sureGorunumu && sureGorunumu.classList.contains('active')) {
                anaSayfayaDonButonu.click();
                return;
            }
            if (aramaGorunumu && aramaGorunumu.classList.contains('active')) {
                aramaGeriButonu.click();
                return;
            }
        }
    });
